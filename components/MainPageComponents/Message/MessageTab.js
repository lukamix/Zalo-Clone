      ...
      <View style={subloginstyles.container}>
        <View style={subloginstyles.header}>
          ...
          <View style={styles.message_icon}>
            <TouchableOpacity>
              <Image
                source={require("../../../assets/images/message/phone.png")}
            ...
            <TouchableOpacity style={styles.info_container}>
              <Image
                source={require("../../../assets/images/message/info.png")}
        ...
        <View style={{ flex: 1 }}>
          <FlatList
            ...
            renderItem={({ item, index }) => {
              ...
              index >= 1
                ? (isRenderedHour =
                    messages[index - 1]?.user._id === receiverid ? true : false)
              ...
              return (
                <View style={styles.message_container}>
                  {isRendered ? null : (
                    <View style={styles.date_time_container}>
                      {/* <Text style = {
                                    ...
                                    styles.message_hour_time_full : styles.message_hour_time}> {process_datetime(item.createdAt)+" "}
                                </Text> */}
                      {this.isToday(item.createdAt) ? null : (
                        <Text style={styles.message_date_time}>
                          {process_datetime(item.createdAt)}{" "}
                        </Text>
                      )}
                    </View>
                  )}
                  ...
                  >
                    {this.isMyMessage(item.user._id) ? null : (
                      <Image
                        style={styles.user_send_image}
                    ...
                    <View
                      style={[
                        styles.message_box,
                        {
                          backgroundColor: this.isMyMessage(item.user._id)
                            ? SUB_COLOR
                            : "white",
                          marginLeft: this.isMyMessage(item.user._id)
                            ? 100
                            : 30,
                          marginRight: this.isMyMessage(item.user._id)
                            ? 0
                            : 100,
                          alignSelf: this.isMyMessage(item.user._id)
                            ? "flex-end"
                            : "flex-start",
                        },
                      ]}
                    >
                      ...
                      {isRenderedHour ? null : (
                        <Text style={styles.message_hour}>{item.hour}</Text>
                      )}
                    </View>
                  </View>
                </View>
          ...
          <View style={styles.message_footer}>
              ...
              {this.state.isTexting ? null : (
                <View style={styles.message_input_button}>
                  <TouchableOpacity
                    ...
                    onPress={this.onPressSmileButton}
                  >
                    <Image
                      source={require("../../../assets/images/message/smile.png")}
                      style={styles.smile_button}
                    />
                  </TouchableOpacity>
            ...
            <TouchableOpacity
              onPress={async () => {this.onPressSendButton(chatID)
                message.push({
                  content: "Ok",
                  createdAt: "",
                  user: {_id: "" }
            ...
            >
              <Image
                source={require("../../../assets/images/message/send.png")}
          ...
          {this.state.isrenderEmoji ? (
            <View>
              <FlatList
                style={styles.emoji_flatlist}
                data={
                  this.state.EmojiChoosingtab == 1
                    ? EMOJI[0]["smiley"]
                    : this.state.EmojiChoosingtab == 2
                    ? EMOJI[0]["people"]
                    : this.state.EmojiChoosingtab == 3
                    ? EMOJI[0]["animals"]
                    : this.state.EmojiChoosingtab == 4
                    ? EMOJI[0]["foods"]
                    : this.state.EmojiChoosingtab == 5
                    ? EMOJI[0]["activities"]
                    : this.state.EmojiChoosingtab == 6
                    ? EMOJI[0]["travels"]
                    : this.state.EmojiChoosingtab == 7
                    ? EMOJI[0]["objects"]
                    : EMOJI[0]["Symbols"]
                }
                extraData={this.state}
              ...
              <View style={styles.emoji_footer}>
                ...
                <TouchableOpacity
                  ...
                  onPress={this.onPressAnimalsTab}
                >
                  <Text style={styles.emoji_tab_text}>🐻</Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                <TouchableOpacity
                  ...
                  onPress={this.onPressActivitiesTab}
                >
                  <Text style={styles.emoji_tab_text}>🎮</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  onPress={this.onPressTravelsTab}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={this.onPressObjectsTab}
                >
                  <Text style={styles.emoji_tab_text}>🎉</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  onPress={this.onPressSymbolsTab}
                >
                  <Text style={styles.emoji_tab_text}>❤️</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.emoji_tab}
                  onPress={() => {
                    if (this.state.MessageSend.length >= 4) {
                      this.setState({
                        isTexting: true,
                        MessageSend: this.state.MessageSend.substr(
                          0,
                          this.state.MessageSend.length - 2
                        ),
                      });
                    } else {
                      this.setState({
                ...
                >
                  <Image
                    source={require("../../../assets/images/message/delete.png")}
